    ...
    <div id="gallery">
        <form >
	        <div class="select-option">
                <font-awesome-icon :icon="['fas', 'caret-square-down']" @click="changingNumber = true" />
                <span class="current-option">{{ onPage }}</span>
                <div class="options" :class="{active : changingNumber}">
                    <span class="option" v-for="option in options" :key="option" @click="onPage = option, changingNumber = false">
                         {{ option }}
                    </span>
                </div>
