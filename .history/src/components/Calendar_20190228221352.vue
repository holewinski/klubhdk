    ...
    <transition name="rotate" mode="out-in">
      <div class="cal-days" :key="activeMonth+'month'">
        <div class="day prev" v-for="p in weekDay-1" :key="months[prevMonth]+'-'+p">
          <template v-if=" windowWidth > 991 ">{{months[prevMonth] }}</template>
          <template v-else>{{ months[prevMonth].substring(0,3) }}</template>
        </div>
        ...
        >
          ...
          >
            <div
              @click="eventShow = true, activeEvent = events[i[2]], firstRender = true"
              class="number"
            >{{ i[1] }}</div>
          </div>
        ...
        <div class="day next" v-for="n in 7 - nextNumber" :key="months[nextMonth]+'-'+n">
          <template v-if=" windowWidth > 991 ">{{months[nextMonth] }}</template>
          <template v-else>{{ months[nextMonth].substring(0,3) }}</template>
        </div>
      </div>
    </transition>
    <transition name="slide-fade">
      <template v-if="firstRender">
        <div class="event" v-show="eventShow" :key="'event-box'" :class="activeEvent.typ.key">
...
export default {
  ...
  watch: {
    activeYear: function(newValue, oldValue) {
      this.changeActualEvents();
      const arr = this.events;
      arr.forEach(item => {
        switch (item.typ) {
          case "donacja":
            item.typ = this.typy.donacja;
            break;
          case "impreza":
            item.typ = this.typy.impreza;
            break;
          case "spotkanie":
            item.typ = this.typy.spotkanie;
            break;
          default:
            break;
        }
        switch (item.lokal) {
          case "nowogrodzka":
            item.loakl = this.miejsca.nowogrodzka;
            break;
          case "siedziba":
            item.lokal = this.miejsca.siedziba;
            break;
          case "centralny":
            item.lokal = this.miejsca.centralny;
            break;
          case "utk":
            item.lokal = this.miejsca.utk;
            break;
          default:
            break;
        }
  ...
  methods: {
    changeActualEvents() {
      const a = new Function(
        "object",
        "return object.r" + this.activeYear + ";"
      );
      this.actualEvents = a(this.events);
    },
    createEventsArrary() {
      this.eventsArray = [];
