<template>
    <div id="slide-show" v-on:mousemove="mouseMove()">
        <div id="box" >
            <div id="back" :class="{'dis-button' : activeSlide == 1}" @click="changeSlide(-1)">
                <font-awesome-icon :icon="['fas', 'chevron-left']" />
            </div>
            <div :class="{loading : isLoading == true, mouseover : mouseover }" id="slide" @mouseover="mouseover = true" @mouseleave="mouseover = false" >
                <img :src="src">
                <div id="panel">
                    <font-awesome-icon :icon="['fas', 'play']" />
                    {{ activeSlide }}/{{ gallery.number }}
                    <font-awesome-icon :icon="['fas', 'times']" @click="changeActiveGallery($event.target)"/>
                </div>
    ...
    methods:{
        slideShow(){
            setInterval(function(){ 
                if(activeSlide == this.gallery.number){
                    clclearInterval(interval);
                }
                else{
                    activeSlide++;
            }},3000)
